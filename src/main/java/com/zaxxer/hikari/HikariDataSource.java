 ...
 * HikariCP的数据源
 * DataSource：一直往上是jdk的数据源配置接口 CommonDataSource
 * Closeable： 资源释放，AutoCloseable
...
public class HikariDataSource extends HikariConfig implements DataSource, Closeable
{
   ...
   /**
    * @guodf  下面代码中会有实际逻辑来理解这部分区别
    * 这里的两个HikariPool的不同取值代表了不同的配置方式：
    * 配置方式一：当通过有参构造new HikariDataSource(HikariConfig configuration)
    *    来创建HikariDataSource时，fastPathPool 和 pool 是非空且相同的；
    * 配置方式二：当通过无参构造new HikariDataSource()来创建HikariDataSource
    *    并手动配置时，fastPathPool 为空，pool 不为空（在第一次 getConnectionI() 时初始化）
    */
   ...
   /**
    * 默认构造器
    * 主要意思是：第一次访问getConnection()的时候，会把连接池启动，同时所有的配置将不能更改
    * 如果要更改，只有通过 HikariConfigMXBean 方法来
    */
