 ...
 * Hibernate 4.3的连接提供程序。
 * @author Brett Wooldridge, Luca Burgazzoli
 */

/**
 * 整体基于hibernate实现的封装
 * ConnectionProvider: ConnctionProvider是连接提供者，hibernate的数据库连接都来自它
 * Configurable: 通用配置属性
 * Stoppable: 连接停止的时候得到通知服务
 */
public class HikariConnectionProvider implements ConnectionProvider, Configurable, Stoppable
{
   ...
   public HikariConnectionProvider()
   {
      ...
      /**
       * 此处限制此处的provider仅支持hibernate版本必须是4.3.6以及以下
       * com.zaxxer.hikari.hibernate.HikariConnectionProvider这个路径下的类不能适配 Hibernate 4.3.6以及以上的版本
       * 如果要使用，需要引用  org.hibernate.hikaricp.internal.HikariCPConnectionProvider
       */
